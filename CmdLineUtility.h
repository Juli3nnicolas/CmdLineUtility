...
namespace CLU
{
	...
	bool ASyncExecute(const char* _cmd, const char* _workingDir = nullptr, bool _hideWindow = true, ProcessPriority _prio = NORMAL);

	// Same as Execute but termination is user managed
	ProcessID PermanentExecute(const char* _cmd, const char* _workingDir = nullptr, bool _hideWindow = true, ProcessPriority _prio = NORMAL);

	// Executes cmd in a dos shell then waits until termination
	// Returns true on successful command line execution.
	bool SyncExecute(const char* _cmd, const char* _workingDir = nullptr, bool _hideWindow = true, ProcessPriority _prio = NORMAL);
