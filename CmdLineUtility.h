...
namespace CLU
{
	...
	bool ASyncExecute(const char* _cmd, const char* _workingDir = nullptr, std::string* _cmdOutputBuf = nullptr, ProcessPriority _prio = NORMAL);

	// Same as Execute but termination is user managed
	ProcessID PermanentExecute(const char* _cmd, const char* _workingDir = nullptr, std::string* _cmdOutputBuf = nullptr, ProcessPriority _prio = NORMAL);
	...
	//
	bool ReadAsyncCmdOutput();

	void StopAsyncCmdOutputRead();
	...
	public:
		...
		// It is used by the execute commands to setup the output process
